    ...
    <div class="maintenance-page">
        ...
        @if(session('success'))
            <div style="background: #e8f5e9; border: 1px solid #c8e6c9; color: #2e7d32; padding: 16px; border-radius: 16px; margin-bottom: 10px; font-weight: 600;">
                {{ session('success') }}
            </div>
        @endif
        ...
        <div class="maintenance-hero">
            ...
            <div class="maintenance-actions">
                <a href="/admin/pengajuan-maintenance" class="maintenance-request-btn">
                    ...
                    <span class="maintenance-count">{{ $summary['waiting'] }}</span>
                </a>
            ...
            <div class="maintenance-list" id="maintenanceList">
                ...
                @foreach($maintenances as $item)
                    @php
                        ...
                        if($item->status === 'process') { $statusLabel = 'Dalam Proses'; }
                        elseif($item->status === 'done') { $statusLabel = 'Selesai'; }
                        
                        // PERUBAHAN KEAMANAN: Menggunakan Nullsafe (?->) agar tidak crash jika relasi kamar kosong
                        $nomorKamar = $item->kamar?->nomor_kamar ?? '-';
                        $avatarKamar = is_numeric($nomorKamar) ? sprintf('%02d', $nomorKamar) : $nomorKamar;
                    @endphp

                    <div class="maintenance-item" data-name="kamar {{ $avatarKamar }} {{ strtolower($item->title) }}" data-status="{{ $item->status }}">
                        <div class="maintenance-avatar">{{ $avatarKamar }}</div>

                        <div class="maintenance-main">
                            <h3>Kamar {{ $avatarKamar }}</h3>
                            <p>{{ $item->title }}</p>

                            <div class="maintenance-meta">
                                <span class="maintenance-badge">{{ $statusLabel }}</span>
                                <span class="maintenance-date">
                                    {{ \Carbon\Carbon::parse($item->date)->translatedFormat('j F Y') }} · Rp {{ number_format($item->cost, 0, ',', '.') }}
                                </span>
                            </div>
                        </div>

                        <div class="maintenance-item-actions">
                            <a href="/admin/maintenance/{{ $item->id }}/edit" class="maintenance-btn maintenance-btn-edit">Edit</a>
                            
                            <form action="/admin/maintenance/{{ $item->id }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')" style="display: inline;">
                                @csrf
                                @method('DELETE')
